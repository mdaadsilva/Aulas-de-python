...
def cadastrar_divida():
    ...
    for divida in dados:
        if divida["nome"].lower() == nome.lower() and divida["mes"] == mes:
            print(f"\n⚠️ Cliente já possui uma dívida no mês {mes}.\n")
            return
...
def listar_dividas():
    ...
    for i, d in enumerate(dados, 1):
        ...
        print(f"{i}. {d['nome']} - Total: R$ {d['valor_total']:.2f} - Pago: R$ {d['valor_pago']:.2f} - Restante: R$ {restante:.2f} - {d['mes']} - {status}")
    print()

def buscar_cliente():
    nome = input("Nome do cliente para buscar: ").strip()
    dados = carregar_dados()
    encontrados = [d for d in dados if d["nome"].lower() == nome.lower()]
    ...
    for d in encontrados:
        restante = d["valor_total"] - d["valor_pago"]
        status = "QUITADA" if restante <= 0 else "PENDENTE"
        print(f"Cliente: {d['nome']} - Total: R$ {d['valor_total']:.2f} - Pago: R$ {d['valor_pago']:.2f} - Restante: R$ {restante:.2f} - Mês: {d['mes']} - Status: {status}")
    print()

def pagar_divida():
    nome = input("Nome do cliente para pagamento: ").strip()
    dados = carregar_dados()
